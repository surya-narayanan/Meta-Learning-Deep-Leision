...
def train_vgg():
  ...
  #Choose certain classes to train and test on
  ...
  X_test, Y_test = sample_from(X_test, Y_test, k, select_classes, frac_data = config.frac_data) 
  ...
  history = new_model.fit(X_train, Y_train, batch_size=2**config.log_batch_size, epochs=2**config.logepochs, validation_data=(X_val, Y_val))
